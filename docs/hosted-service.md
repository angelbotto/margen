...
## Choose agents and connect your identity

The installer page separates three decisions: OS/terminal, local agents, and the publishing account. Apple, Linux and Windows choices generate commands for the corresponding shell. The Windows choice offers an explicit WSL option; this installs inside the Linux environment.

Select one or more agents. The command passes `--agents codex,claude,hermes` to the Python updater (a subset is accepted), or `-Agents 'codex,hermes'` to PowerShell. Only selected integrations are created. `margen update` remembers that selection, including scheduled updates. A first CLI install without a selection preserves the historical default of all three agents. Existing integrations are not removed when deselected; select all agents you want to keep updated. Independent skill folders remain untouched. This installs the Margen skill, not the agent application.

Installing does not authenticate or transfer ownership. Sign in to the portal with your own Google account or verified email, open **Connect an agent**, and create a personal token with a recognizable device label. Run the displayed connection command locally: it checks your verified email against the token's account and asks for the secret through a hidden terminal prompt. Never put the token in an agent chat or an artifact. A connection is shared by agents running under the same local OS user; a different person must connect their own account. `margen status` verifies the connection. Choosing an agent or a company theme does not change ownership or access permissions.
