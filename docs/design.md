...
The reader inherits document tokens. Its three-tool dock contains Comments with the count on its speech bubble, Share, and Preferences, in that order. See [reader controls](reader-controls.md) for contextual actions and point anchoring. Comments use a compact writing surface with author, privacy and send; optional type, session and anchor context are disclosed. Appearance uses a 380 px panel with a searchable single-column theme list, independent mode control and separate typography/sound tabs.
